...
Right now the agent classifies a message and a dispatcher acts on it. If the dispatcher thinks the classification is wrong they just fix it manually and move on. That correction disappears. Nobody learns from it.

I'd build a simple thumbs up / thumbs down on each triage result so dispatchers can flag when the agent got it wrong and record what the correct answer should have been. Nothing fancy. Over a few weeks of real traffic you'd have a labelled dataset of actual production messages that reflects the edge cases your 20-message benchmark doesn't cover. You'd also start to see patterns: maybe the agent consistently misroutes a particular type of complaint, or struggles with a specific phrasing that customers use. That's the kind of signal that tells you exactly where to tighten the prompt rather than having to guess.
